...
async function fetchSlots() {
    ...
    try {
        const res = await fetch(GAS_URL, {
            ...
            body: JSON.stringify({
                action: "getSlots",
                calendarId: CALENDAR_IDS[currentType]   // ← ここで切り替え！
            })
        });
    ...
    slots.forEach(slot => {
        list.innerHTML += `
            <div class="slot-item">
                ...
                <button onclick="reserve('${slot.id}', '${slot.start}', '${slot.end}')">予約</button>
            </div>
        `;
    });
}
...
async function reserve(slotId, start, end) {
    ...
    const userName = "ゲスト"; // 後で LIFF の名前に差し替え可
